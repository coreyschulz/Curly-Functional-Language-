...
(define (exp-i->c [a : ExpI] [super-name : Symbol]) : Exp
  (local [(define (recur expr)
    ...
    (type-case ExpI a
      ...
      [(argI) (argE)]
      [(thisI) (thisE)]
      [(castI sym oe) (castE sym (recur oe))]
      ...
      [(superI method-name arg-expr)
       (ssendE (thisE)
               ...
               (recur arg-expr))]
      [(if0I cond fst snd)
       (if0E (recur cond) (recur fst) (recur snd))])))
...
(module+ test
  ...
  (test (exp-i->c (numI 10) 'Object)
        (numE 10))
  (test (exp-i->c (castI 'Object (numI 10)) 'Object)
        (castE 'Object (numE 10)))
  ...
  (type-case Class (find classes-not-flat name)
    [(classC field-names methods parent)
     (type-case Class (flatten-super name classes-not-flat i-classes)
       [(classC super-field-names super-methods super-parent)
        (classC
